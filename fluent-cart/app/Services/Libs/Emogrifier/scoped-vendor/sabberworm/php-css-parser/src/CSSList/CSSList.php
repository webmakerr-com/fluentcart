...
use FluentEmogrifier\Vendor\Sabberworm\CSS\Comment\Comment;
...
use FluentEmogrifier\Vendor\Sabberworm\CSS\Property\Charset;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Property\CSSNamespace;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Property\Import;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Property\Selector;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Renderable;
use FluentEmogrifier\Vendor\Sabberworm\CSS\RuleSet\AtRuleSet;
use FluentEmogrifier\Vendor\Sabberworm\CSS\RuleSet\DeclarationBlock;
use FluentEmogrifier\Vendor\Sabberworm\CSS\RuleSet\RuleSet;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Settings;
use FluentEmogrifier\Vendor\Sabberworm\CSS\Value\CSSString;
    ...
    private static function parseAtRule(ParserState $oParserState)
    {
        ...
        if ($sIdentifier === 'import') {
            ...
            if ($sPrefix !== null && !\is_string($sPrefix)) {
                throw new UnexpectedTokenException('Wrong namespace prefix', esc_html( $sPrefix ), 'custom', esc_html( $iIdentifierLineNum ));
            }
            if (!($mUrl instanceof CSSString || $mUrl instanceof URL)) {
                throw new UnexpectedTokenException('Wrong namespace url of invalid type', esc_html($mUrl), 'custom', esc_html($iIdentifierLineNum));
            }
            ...
            if (!$mSel instanceof Selector) {
                if (!Selector::isValid($mSel)) {
                    throw new UnexpectedTokenException(
                        sprintf(
                        /* translators: %s is the selector regex pattern */
