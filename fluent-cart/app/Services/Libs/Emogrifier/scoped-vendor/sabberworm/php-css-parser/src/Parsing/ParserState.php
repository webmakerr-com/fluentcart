...
namespace FluentEmogrifier\Vendor\Sabberworm\CSS\Parsing;
...
class ParserState
{
    ...
    {
        if ($this->isEnd()) {
            throw new UnexpectedEOFException('', '', 'identifier', esc_html($this->iLineNo));
        }
        $sResult = $this->parseCharacter(\true);
        if ($sResult === null) {
            throw new UnexpectedTokenException(esc_html($sResult), esc_html($this->peek(5)), 'identifier', esc_html($this->iLineNo));
        }
        $sCharacter = null;
        ...
        if ($this->peek() === '\\') {
            ...
            if ($this->strlen($sUnicode) < 6) {
                ...
                if (\preg_match('/\\s/isSu', $this->peek())) {
                    if ($this->comes('FluentEmogrifier\\Vendor\\r\\n')) {
                        $this->consume(2);
                    } else {
                        $this->consume(1);
                    }
    ...
    public function consume($mValue = 1)
    {
        if (\is_string($mValue)) {
            ...
            if ($this->iCurrentPosition + $mValue > $this->iLength) {
                throw new UnexpectedEOFException(esc_html($mValue), esc_html($this->peek(5)), 'count', esc_html($this->iLineNo));
            }
    ...
    public function consumeExpression($mExpression, $iMaxLength = null)
    {
        ...
        throw new UnexpectedTokenException(esc_html($mExpression), esc_html($this->peek(5)), 'expression', esc_html($this->iLineNo));
    }
    /**
    ...
    public function consumeUntil($aEnd, $bIncludeEnd = \false, $consumeEnd = \false, array &$comments = [])
    {
        ...
        throw new UnexpectedEOFException('One of ("' . \esc_html(implode('","', $aEnd)) . '")', esc_html($this->peek(5)), 'search', esc_html($this->iLineNo));
    }
    /**
     * @return string
     */
