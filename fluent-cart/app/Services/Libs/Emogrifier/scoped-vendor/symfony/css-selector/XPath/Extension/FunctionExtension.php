...
class FunctionExtension extends AbstractExtension
{
    ...
    public function translateNthChild(XPathExpr $xpath, FunctionNode $function, bool $last = \false, bool $addNameTest = \true) : XPathExpr
    {
        try {
            ...
            throw new ExpressionErrorException(\sprintf('Invalid series: "%s".', \esc_html(implode('", "', $function->getArguments()))), 0, esc_html($e));
        }
        $xpath->addStarPrefix();
        if ($addNameTest) {
            $xpath->addNameTest();
    ...
    public function translateContains(XPathExpr $xpath, FunctionNode $function) : XPathExpr
    {
        ...
        foreach ($arguments as $token) {
            if (!($token->isString() || $token->isIdentifier())) {
                throw new ExpressionErrorException('Expected a single string or identifier for :contains(), got ' . \esc_html(implode(', ', $arguments)));
            }
        }
    ...
    public function translateLang(XPathExpr $xpath, FunctionNode $function) : XPathExpr
    {
        ...
        foreach ($arguments as $token) {
            if (!($token->isString() || $token->isIdentifier())) {
                throw new ExpressionErrorException('Expected a single string or identifier for :lang(), got ' . \esc_html(implode(', ', $arguments)));
            }
        }
